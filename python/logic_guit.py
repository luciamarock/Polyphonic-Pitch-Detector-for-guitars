...
# serve per dare peso alla detection monofonica, generato da C++
    ...
    for j in range(len(data.T)):
        if (j > 0 and j < (len(data.T)-1)):
            if (data[i, j]-data[i, j-1] > 0 and data[i, j]-data[i, j+1] > 0):
                # se e un punto di massimo mi scrivo il valore RTFI
...
# for evaluating the various conditions
    for j in range(len(data.T)):
        ...
        if (abs(freq[j]-fund[i]) < 0.01 and fund[i] < 320):
            # se la detection monofonica rileva note plausibili ci scrivo il valore della fft
            vectmono[j] = data[i, j]/170
        # CONDIZIONE 1 basata sugli armonici
        if (j < (len(data.T)-20) and 
        vectnote[j] > 0.0 and 
        allowance > 0.0 and 
        vectnote[j+12] < vectnote[j]*Pshp*(1.0+Pthp) and 
        vectnote[j+12] > vectnote[j]*Pshp*(1.0-Pthp) and 
        vectnote[j+19] < vectnote[j]*Pshs*(1.0+Pths) and 
        vectnote[j+19] > vectnote[j]*Pshs*(1.0-Pths)):
            # valutare se e il caso di correggere questa assegnazione sulla base di un vectmono adiacente
            vectnote[j] = data[i, j]/170
            ...
            if (exist[j] == 4 and dummywarn == 0):
                print(
                    '--> nota {} fissata con condizione 1 @ nchunk = {}'.format(names[j], str(i+1)))
            ...
            if (j < 19 and exist[j] < 2):
                if (datafft[i, j] < datafft[i, j+19]/6.0 or datafft[i, j+19] < datafft[i, j]/6.0):
                    ...
                    condizione = 11
                    vectcond[j] = condizione
                    print('{}. ammazzato {} con la condizione 11' .format(
                        str(i+1), str(names[j])))
        # CONDIZIONE 2    basata sull'algoritmo di detection della waveform
        # and vectnote[j+19]>0.03
        elif (j < (len(data.T)-20) and 
        existsum < 5 and 
        vectmono[j] > 0.0 and 
        allowance > 0.0 and 
        vectnote[j+12] < vectmono[j]*Pshp*(1.0+Pthp) and 
        vectnote[j+12] > vectmono[j]*Pshp*(1.0-Pthp)):
            ''' STAMPA MOTIVI DECADIMENTO COND 1 '''  # existsum fa in modo che la det temp viene ignorata quando 
            #viene riconosciuta la prima nota + 1 un'altra sospettata di esistere
            if (vectnote[j] > 0.0 and vectnote[j+12] >= vectnote[j]*Pshp*(1.0+Pthp)):
                print('{}. primo armonico di {} sfora la soglia superiore' .format(
                    str(i+1), names[j]))
            elif (vectnote[j+12] > 0.0 and vectnote[j+12] <= vectnote[j]*Pshp*(1.0-Pthp)):
                print('{}. primo armonico di {} sfora la soglia inferiore' .format(
                    str(i+1), names[j]))
            elif (vectnote[j] > 0.0 and vectnote[j+19] >= vectnote[j]*Pshs*(1.0+Pths)):
                print('{}. secondo armonico di {} sfora la soglia superiore' .format(
                    str(i+1), names[j]))
            elif (vectnote[j+19] > 0.0 and vectnote[j+19] <= vectnote[j]*Pshs*(1.0-Pths)):
                print('{}. secondo armonico di {} sfora la soglia inferiore' .format(
                    str(i+1), names[j]))
        ...
        elif (j > 27 and j < 38 and vectnote[j] > 0.0 and 
        vectnote[j] > data[i, j-12]/170*1.15 and 
        vectnote[j] > data[i, j-19]/170*1.15 and 
        allowance > 0.0 and 
        vectnote[j+12] > vectnote[j]*0.75 and 
        vectnote[j+12] < vectnote[j]*1.25):
            ...
            if (exist[j] == 4 and dummywarn == 0):
                print(
                    '--> nota {} fissata con condizione 3 @ nchunk = {}'.format(names[j], str(i+1)))
                # print('--> j = {}'.format(str(j)))
                dummywarn = 1
        elif (j > 37 and j < 51 and 
        vectnote[j] > 0.0 and 
        vectnote[j] > data[i, j-12]/170*1.15 and 
        vectnote[j] > data[i, j-19]/170*1.15 and 
        allowance > 0.0 and 
        (datafft[i, j]+datafft[i, j+12]+datafft[i, j+19]) > (datafft[i, j-12]+datafft[i, j]+datafft[i, j+7])*0.75 and 
        (datafft[i, j]+datafft[i, j+12]+datafft[i, j+19]) > (datafft[i, j-19]+datafft[i, j-7]+datafft[i, j])*0.75):
            vectnote[j] = data[i, j]/170
            memoria[j] = data[i, j]/170
            condizione = 4
            vectcond[j] = condizione
            if (exist[j] == 4 and dummywarn == 0):
                print(
                    '--> nota {} fissata con condizione 4 @ nchunk = {}'.format(names[j], str(i+1)))
                # print('--> j = {}'.format(str(j)))
                dummywarn = 1

                ''' CONDIZIONI POLIFONICHE '''

        elif (j > 6 and j < 39 and 
        vectnote[j] > 0.93*(vectnote[j-7]+vectnote[j+5])*0.5 and 
        exist[j-7] > 0 and datafft[i, j+12] > 0.66*datafft[i, j-7]):
            vectnote[j] = data[i, j]/170
            memoria[j] = data[i, j]/170
            ...
            condizione = 5
            vectcond[j] = condizione
        elif (j > 15 and j < 47 and 
        ((exist[j-16] > 0 and exist[j-4] > 0 and exist[j+3] > 0 and 
        vectnote[j] > 0.93*(vectnote[j-16]+vectnote[j-4]+vectnote[j+3])/3.) or 
        (exist[j-15] > 0 and exist[j-3] > 0 and exist[j+4] > 0 and 
        vectnote[j] > 0.93*(vectnote[j-15]+vectnote[j-3]+vectnote[j+4])/3.))):
            if exist[j] < 4:
        ...
        else:            # commento il primo if elif perche' si riferisce alla condizione monofonica, 
            # invece adesso io studio le cond polifoniche
            ...
            elif (exist[j] > 3 and datafft[i, j]*3.75 > 0.03 and 
            data[i, j]-data[i, j-1] > 0 and data[i, j]-data[i, j+1] > 0):
                vectnote[j] = data[i, j]/170
                memoria[j] = data[i, j]/170
...
# plot-only variables
    ...
    for k in range(len(vectcond)):
        if vectcond[k] != 1.5:
            # annotation value, (coordinates)
            plt.annotate(str(vectcond[k]), (freq[k], 1.5))
        if jvect[k] != 1.4:
    ...
    plt.text(1000, 1.9, "nchunck = " + str(i+1),
             {'color': 'black', 'fontsize': 16})
    plt.text(1000, 1.75, "condizione " + str(condizione),
             {'color': 'black', 'fontsize': 16})
    plt.text(700, 1.9, "time = " + str(round(time)) +
             " ms", {'color': 'black', 'fontsize': 16})
    plt.text(100, 1.9, "attack = " + str(round(attack, 3)),
             {'color': 'black', 'fontsize': 16})
    plt.text(100, 1.75, "existsum =  " + str(existsum),
             {'color': 'black', 'fontsize': 16})
